...
for row in spamreader:
    ...
    timediff3 = int(row[2]) #2 is duration
...
print('Vectorization...')
num_features = len(chars)+3
print('num features: {}'.format(num_features))
X = np.zeros((len(sentences), maxlen, num_features), dtype=np.float32)
y_a = np.zeros((len(sentences), len(target_chars)), dtype=np.float32)
y_t = np.zeros((len(sentences),2), dtype=np.float32)
...
#        X[i, t+leftpad, len(chars)+2] = sentence_t2[t]/divisor2
        X[i, t+leftpad, len(chars)+2] = sentence_t3[t]/divisor3
...
with open("output_files/folds/matrix.txt", "w") as text_file:
    for i in range(0,20):
        for j in range(0,maxlen):
            ...
            text_file.write(row+'\n')
        row = ''
        for k in range(0,num_features - 3):
            row+=str(y_a[i,k])
            row+=','
        text_file.write(row+'\n')
...
time_output = Dense(2, init='glorot_uniform', name='time_output')(b2_2)
...
def encodePrediction(sentence, times, times2, times3, maxlen=maxlen):
...
#        X[0, t+leftpad, len(chars)+2] = times2[t]/divisor2
        X[0, t+leftpad, len(chars)+2] = times3[t]/divisor3
    return X
...
#                predicted_t2.append(y_t2)
            ...
            if len(predicted) > 0:
                ...
                output.append(sum(times[:prefix_size]) + sum(predicted_t)) #changed
