...
#train
model.fit(X, {'act_output':y_a, 'time_output':y_t}, validation_split=0.2, verbose=2, callbacks=[early_stopping, model_checkpoint, lr_reducer], batch_size=maxlen, nb_epoch=1)
...
with open('output_files/results/results-{}.csv'.format(filename), 'w') as csvfile:
    ...
    for line, times, times2, times3,times4, times5, meta1, meta2 in zip(lines, lines_t, lines_t2, lines_t3, lines_t4, lines_t5, lines_m1, lines_m2):
