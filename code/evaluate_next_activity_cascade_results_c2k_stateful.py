...
def encode(sentence, times, times2, times3, maxlen=maxlen):
    ...
    X = np.zeros((1, 1, num_features), dtype=np.float32)
    leftpad = maxlen-len(sentence)
    previous_char = 0
    for t, char in enumerate(sentence):
        for c in chars:
            if c==char:
                X[0, 0, previous_char] = 0
                X[0, 0, char_indices[c]] = 1
                previous_char = char_indices[c]
        X[0, 0, len(chars)] = t+1
        X[0, 0, len(chars)+1] = times[t]/divisor
        X[0, 0, len(chars)+2] = times2[t]/divisor2
        X[0, 0, len(chars)+3] = times3[t]/divisor3
    return X
