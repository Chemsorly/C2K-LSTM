...
X = np.zeros((len(sentences), 1, num_features), dtype=np.float32)
...
for i, sentence in enumerate(sentences):
    ...
    for t, char in enumerate(sentence):
        for c in chars:
            if c==char:
                X[i, 0, previous_char] = 0
                X[i, 0, char_indices[c]] = 1
                previous_char = char_indices[c]
        X[i, 0, len(chars)] = t+1
        X[i, 0, len(chars)+1] = sentence_t[t]/divisor
        X[i, 0, len(chars)+2] = sentence_t2[t]/divisor2
        X[i, 0, len(chars)+3] = sentence_t3[t]/divisor3
...
# build the model: 
print('Build model...')
main_input = Input(shape=(1, num_features),batch_shape=(1,1, num_features), name='main_input')
# train a 2-layer LSTM with one shared layer
l1 = LSTM(par_neurons,batch_input_shape=(1, 1, num_features), consume_less='gpu', init='glorot_uniform', return_sequences=True, dropout_W=par_dropout, stateful=True)(main_input) # the shared layer
b1 = BatchNormalization()(l1)
l2_1 = LSTM(par_neurons, consume_less='gpu', init='glorot_uniform', return_sequences=False, dropout_W=par_dropout, stateful=True)(b1) # the layer specialized in activity prediction
b2_1 = BatchNormalization()(l2_1)
l2_2 = LSTM(par_neurons, consume_less='gpu', init='glorot_uniform', return_sequences=False, dropout_W=par_dropout, stateful=True)(b1) # the layer specialized in time prediction
...
class ResetStatesCallback(Callback):
    def on_batch_begin(self, batch, logs={}):
        if X[batch,0,len(chars)] == 1:
            self.model.reset_states()
resetstatescallback = ResetStatesCallback()
...
model.fit(X, {'act_output':y_a, 'time_output':y_t}, validation_split=0.2, verbose=2, callbacks=[early_stopping, model_checkpoint, lr_reducer, resetstatescallback], batch_size=1, nb_epoch=500)